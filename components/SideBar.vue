...
const belongingContent = computed(() => {
  const contents: Record<number, any> = {
    1: {
      ...
      description:
        'Оберіть іконку (навівши на неї, побачите підказку) та позначте на мапі місця, які вам подобаються, не подобаються або які викликають спогади. Додайте коментар із поясненням, і після цього переходьте до розділу «Безпека».',
    },
  }
...
const safetyContent = computed(() => {
  const contents: Record<number, any> = {
    1: {
      ...
      description:
        "Оберіть іконку (навівши на неї, побачите підказку) та позначте на мапі місця, де вам безпечно, небезпечно або спокійно. Додайте коментар із поясненням, і після цього переходьте до розділу «Екологія».",
    },
  }
